...
- [ ] T018 [US1] Swizzle the Docusaurus `Root` component by running `npm run swizzle @docusaurus/theme-classic Root -- --danger` in the `docusaurus-book-site` directory. (SKIPPED by user request)
...
- [ ] T033 Manually test all acceptance criteria from the `spec.md` and verify all tasks are complete. (Requires manual verification by user)
